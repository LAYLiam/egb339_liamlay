...
class CoppeliaRobot(GenericRobotAPI):
    ...
    def wait_joints(self):
        for joint in self.joints:
            while not np.isclose(
                ...
                self.sim.getJointTargetPosition(joint),
                atol=0.01,
            ):
                pass
    ...
    def REF_ikine(self, pos: np.array) -> np.ndarray:
        """
        ...
        assert isinstance(pos,np.ndarray) and pos.shape==(3,), "Provided position must be a numpy array of shape (3,)"

        with open("reference_values.pkl", "rb") as fp:
            data = pickle.load(fp)
            for key, val in data.items():
                if isinstance(key, tuple):
                    if np.isclose(pos, key, atol=10).all():
                        theta = val
                        break
        return theta
    ...
    def REF_GetHomography(self, sorted_calibration_markers):
        ...
        if not isinstance(sorted_calibration_markers,Iterable) and len(sorted_calibration_markers) == 4:
            raise ValueError("calibration markers must be an iterable with exactly 4 elements.")
        if not all([isinstance(elem,mvt.Blob) for elem in sorted_calibration_markers]):
            raise ValueError("All values inside the calibration markers iterable must be of type mvt.blob")
    ...
    def REF_GetObjectXY(self, shapes, homography):
        """
        Converts UV coordinates into XY coordinates using the provided homography for all provided shapes
        ...
        assert isinstance(shapes,Dict), "shapes argument must be a dictionary."

        assert isinstance(homography,np.ndarray) and homography.shape==(3,3), "homography must be a (3,3) numpy array"
    ...
    def REF_SortCalibrationMarkers(self, calibration_markers):
        ...
        if not isinstance(calibration_markers,Iterable) and len(calibration_markers) == 4:
            raise ValueError("calibration markers must be an iterable with exactly 4 elements.")
        if not all([isinstance(elem,mvt.Blob) for elem in calibration_markers]):
            raise ValueError("All values inside the calibration markers iterable must be of type mvt.blob")
        ...
        with open("reference_values.pkl", "rb") as fp:
            ...
            for i in range(4):
                new_blob = mvt.Blob(
                    ...
                    moments="NOT IMPLEMENTED IN REF FUNCTIONALITY",
                    ...
                    color="NOT IMPLEMENTED IN REF FUNCTIONALITY",
                    perimeter_length="NOT IMPLEMENTED IN REF FUNCTIONALITY",
                    contourpoint="NOT IMPLEMENTED IN REF FUNCTIONALITY",
                    circularity="NOT IMPLEMENTED IN REF FUNCTIONALITY"
                )
        ...
        with open("reference_values.pkl", "rb") as fp:
            ...
            for i in range(5):
                new_blob = mvt.Blob(
                    id=data["id"][i],
                    bbox=data["bbox"][i],
                    moments="NOT IMPLEMENTED IN REF FUNCTIONALITY", #data["moments"][i],
                    touch=data["touch"][i],
