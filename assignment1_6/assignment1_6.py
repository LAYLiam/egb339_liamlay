...
def coloured_objects(img: mvt.Image) -> Tuple[int, int, int]:
    ...
    red = (r >= 0.4) and (g <= 0.3) and (b <= 0.3)
    green = (r >= 0.4) and (g <= 0.3) and (b <= 0.3)
    blue = (r >= 0.4) and (g <= 0.3) and (b <= 0.3)

    return red.blobs(), green.blobs(), blue.blobs()

if __name__ == "__main__":
    print(coloured_objects(mvt.Image.Read('sample_image.png')))
    """
    print("")
